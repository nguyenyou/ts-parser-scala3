...
import io.github.nguyenyou.internal.scalajs.transforms.{CleanIllegalNames, UnionToInheritance}
...
object FindProps {
  ...
  sealed trait Res[+E, +T] {
    ...
    def map[U](f: T => U): Res[E, U] = this match {
      ...
      case Res.Many(values)     => Res.Many(values.view.mapValues(f).toMap)
    }
