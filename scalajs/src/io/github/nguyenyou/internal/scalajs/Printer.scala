...
import io.github.nguyenyou.internal.scalajs.transforms.ShortenNames
...
object Printer {
  ...
  private[Printer] class Registry() {
    ...
    private val files = IArray.Builder.empty[(String, String)]

    def write(content: String)(f: PrintWriter => Unit): Unit = {
      
    }

    def result: IArray[(String, String)] =
      files.result()
  }
    ...
    def apply(
      ...
      files.foreach {
        case (ScalaOutput.File(name), members: IArray[Tree]) =>
          reg.write(s"${targetFolder}/${name.unescaped}.scala") { writer =>
        ...
        case (ScalaOutput.Package(name), pkgs) =>
          pkgs.foreach {
            case pkg: PackageTree =>
              apply(scope, parentsResolver, reg, packages :+ name, s"${targetFolder}/${name.unescaped}", pkg)
            case _ => sys.error("i was too lazy to prove this with types")
          }

        case (ScalaOutput.PackageObject, members) if scalaVersion.is3 =>
          reg.write(s"${targetFolder}/${packageScalaFileName}") { writer =>
    ...
    def printTree(
      ...
      tree match {
        case tree: PackageTree =>
          apply(scope, parentsResolver, reg, packageNames :+ tree.name, s"${folder}/${tree.name.value}", tree)
