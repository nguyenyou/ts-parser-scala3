...
object TsQIdent {
  ...
  implicit val decodes: Decoder[TsQIdent] =
    io.circe.generic.semiauto.deriveDecoder
  implicit val ordering: Ordering[TsQIdent] = new Ordering[TsQIdent] {
    override def compare(x: TsQIdent, y: TsQIdent): Int = {
      val xParts = x.parts
      val yParts = y.parts
      var idx = 0
      while (idx < xParts.length && idx < yParts.length) {
        TsIdent.ordering.compare(xParts(idx), yParts(idx)) match {
          case 0 => // continue
          case n => return n
        }
        idx += 1
      ...
      if (xParts.length < yParts.length) -1
      else if (yParts.length < xParts.length) 1
      else 0
    }
  }
...
final case class TsTypeObject(comments: Comments, members: IArray[TsMember])
    extends TsType
    with HasClassMembers
...
final case class TsTypeAsserts(ident: TsIdentSimple, isOpt: Option[TsType])
    extends TsType
...
object TsTypeIntersect {
  ...
  def simplified(types: IArray[TsType]): TsType = {
    val withCombinedObjects = types.partitionCollect {
      ...
      case (objects, rest) =>
        IArray(
          TsTypeObject(NoComments, objects.flatMap(_.members).distinct)
        ) ++ rest
    }
...
final case class TsTypeExtends(tpe: TsType, `extends`: TsType)
    extends TsTypePredicate
