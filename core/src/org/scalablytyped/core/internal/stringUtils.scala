...
object stringUtils {
  ...
  /** pff, scala apparently cares about nested comments, especially when they're
    * not balanced
    */
  def escapeNestedComments(s: String): String =
    (s.indexOf("/*"), s.lastIndexOf("*/")) match {
      ...
      case (start, end) =>
        ...
        val endComment = s.substring(end)
        val escaped: String =
          s.substring(Math.min(end, start + 2), end)
            .replaceAll("/\\*", "/ *")
            .replaceAll("\\*/", "* /")
        startComment + escaped + endComment
    }
  ...
  def joinCamelCase(strings: List[String]): String =
    strings
      ...
      .zipWithIndex
      .map {
        case (x, 0) if x.length > 2 && x(0).isUpper && x(1).isUpper =>
          x.toLowerCase // avoid things like dOM...
  ...
  // Scala.js compatible URI encoding (simplified version)
  def encodeURIComponent(s: String): String = {
    ...
    for (c <- s) {
      c match {
        ...
        case ' '                    => sb.append("%20")
        case _ =>
          val hex = c.toInt.toHexString.toUpperCase
          if (hex.length == 1) sb.append("%0").append(hex)
          else sb.append("%").append(hex)
      }
    }
    sb.toString()
  }
