...
object Name {
  ...
  implicit val OrderedName: Ordering[Name] =
    Ordering[String].on[Name](_.unescaped)
  ...
  /** All names must pass through here, especially including the ones from
    * arbitrary javascript strings.
    *
    * For performance reasons it's not done in the constructor or anything like
    * that
    */
      ...
      case ident =>
        ...
        val patchedChars =
          ident.flatMap {
            ...
            case '\u0000' => "Null"
            case ' '      => " "
            case c if !c.isUnicodeIdentifierPart || c.isControl =>
              unicodeName(c)
            case c => c.toString
          }
        ...
        val fixedSpaces: String = {
          ...
          "Space" * initialSpaces.length + patchedChars.drop(
        ...
        val notTooLong =
          if (fixedSpaces.length > 500) fixedSpaces.take(500) else fixedSpaces

        val legal = notTooLong match {
          case "" => "_empty" // must have a name
          case "-" =>
            "_dash" // `def `-`(d: Double) = d; `-`(d) doesn't do what you would think
          case "_" =>
            "_underscore" // can't import a top level member with this name
