...
object Imported {
  ...
  implicit val encodes: Encoder[Imported] =
    io.circe.generic.semiauto.deriveEncoder
  implicit val decodes: Decoder[Imported] =
...
object Annotation {
  ...
  case class JsImport(
      module: String,
      imported: Imported,
      global: Option[JsGlobal]
  ) extends LocationAnnotation
  case class JsGlobal(name: QualifiedName) extends LocationAnnotation

  object JsGlobal {
    implicit val encodes: Encoder[JsGlobal] =
      io.circe.generic.semiauto.deriveEncoder
    implicit val decodes: Decoder[JsGlobal] =
      io.circe.generic.semiauto.deriveDecoder
  }

  def renamedFrom(
      newName: Name
  )(oldAnnotations: IArray[Annotation]): IArray[Annotation] = {
    val (names, others) =
      oldAnnotations.partition {
        case _: JsName | _: JsNameSymbol | JsBracketCall | (_: LocationAnnotation) =>
          true
        case _ => false
      }
    ...
    val updatedNames: IArray[Annotation] =
      (names, newName) match {
        case (Empty, n @ (Name.APPLY | Name.namespaced)) =>
          sys.error(s"Cannot rename `$n`")
  ...
  implicit lazy val encodes: Encoder[Annotation] =
    io.circe.generic.semiauto.deriveEncoder
  implicit lazy val decodes: Decoder[Annotation] =
    io.circe.generic.semiauto.deriveDecoder
}
