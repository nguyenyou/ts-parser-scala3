    ...
    current match {
      ...
      case TsTypeObject(_, members) =>
        members.flatMap {
          ...
          case _ => Empty
          /** such a recursive link will be broken by [[ExtractInterfaces]] */
        }
    ...
    override def enterTsDecl(scope: TsTreeScope)(x: TsDecl): TsDecl =
      x match {
        ...
        /** Given this:
          ...
          * }}}
          * In Typescript these two are structurally equal while in nominally typed Scala we need to cast a `Foo` to a
          * `Bar` For this reason we rewrite to a type alias to keep it as convenient.
          *
          * {{{
          * interface Foo {bar: number}
          * type Bar = Foo
          * }}}
          * Note that we rewrite interfaces which extends one type, not more. The reason is that scala wont't let you
        ...
        /** There is also a case where we prefer interfaces:
          ...
          * type Foo = { bar: number }
          * }}}
          * We would end up with the following given the logic in `ExtractInterfaces`: {{{scala trait Anon_Bar { bar:
          * Double} trait Foo extends Anon_Bar }}}
          *
